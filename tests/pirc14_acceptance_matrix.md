...
| AT-004 | REQ-002, REQ-005 | API/browser | Initial pull or push returns a directional unified diff and confirmation token without changing either side; the browser renders it through the professional diff library with source line numbers, red removals, green additions, and change counts; valid confirmation applies it, while stale/mismatched confirmation performs zero writes | DEV-03, DEV-07, REWORK-04, REWORK-06 |
| AT-005 | REQ-003 | browser | One ordinary Markdown file can be opened and handled in a single work area with Difference, Markdown source, and secondary Render modes; rendering uses the standard CommonMark adapter rather than a handwritten parser; catalog, focus, and grouped sync remain available | DEV-06, REWORK-04, REWORK-05 |
